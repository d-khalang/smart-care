...
class Reporter():
    def __init__(self, config: Config):
        ...
        self.client = InferenceClient(api_key=config.LLM_API_KEY)
    ...
    def generate_llm_insight(self, report):
        ...
        message = f"""
            Here is the collected data for plant {report.get('plant_id', 'unknown')} which is lettuce:

            Temperature: {report.get('averages', {}).get('temperature', 'N/A')} °C
            Light: {report.get('averages', {}).get('light', 'N/A')} μmol/m²/s
            pH: {report.get('averages', {}).get('ph', 'N/A')}
            Soil Moisture: {report.get('averages', {}).get('soil_moisture', 'N/A')} %
