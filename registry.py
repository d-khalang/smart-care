...
def camel_snake_handler_for_dict(input_dict:dict, from_type: Literal["camel", "snake"]):
    if from_type == "camel":
        output_dict = {camel_to_snake(key):value for key, value in input_dict.items()}
    elif from_type == "snake":
        output_dict = {to_lower_camel_case(key):value for key, value in input_dict.items()}
    else:
        output_dict = {}
    return output_dict
